...
export default function CardStack() {
  ...
  useEffect(() => {
    if (
      ...
      (currentIndex === cardData.length - 1 && direction === 1)
    ) {
      setShowHeartsOut(true);
      setTimeout(() => {
        setShowHeartsOut(false);
        ...
        <AnimatePresence initial={false}>
          ...
          >
            <ValentineCard
              ...
              isSpecialCard={
                currentIndex === 0 || currentIndex === cardData.length - 1
              }
              onCorrectAnswer={
                currentIndex === 0 ? handleCorrectAnswer : undefined
              }
            />
          </motion.div>
        </AnimatePresence>
      </div>
      {quizCompleted && currentIndex === 0 && (
